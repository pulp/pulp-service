...
class ContentScanSerializer(serializers.Serializer, ValidateFieldsMixin):
    ...
    def validate(self, data):
        ...
        repo_ver = data.get("repo_version", None)
        pkg_json = data.get("package_json", None)
        if bool(repo_ver) == bool(pkg_json):
    ...
    def _validate_rpm_repo_expected_fields(self, repo):
        ...
        if repo.pulp_labels.get(OSV_RH_ECOSYSTEM_LABEL) == "Red Hat":
            ...
            return (
                repo.pulp_labels.get(OSV_RH_ECOSYSTEM_CPES_LABEL)
                and len(repo.pulp_labels[OSV_RH_ECOSYSTEM_CPES_LABEL]) > 0
            )
        return False
