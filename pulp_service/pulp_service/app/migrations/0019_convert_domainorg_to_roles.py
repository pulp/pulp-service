...
def convert_domainorgs_to_roles(apps, schema_editor, set_permissions=True, include_readonly_policies=True):  # noqa: ARG001
    ...
    # Prefetch domains with only("pk"): full Domain rows decrypt storage_settings and can raise
    # InvalidToken; the migration only needs pk and the domain FK.
    domain_qs = Domain.objects.only("pk")
    queryset = DomainOrg.objects.select_related("user", "group").prefetch_related(
        Prefetch("domains", queryset=domain_qs)
    )
    for domain_org in queryset.iterator(chunk_size=500):
        ...
        if user is None and group is None:
            if domain_org.org_id:
                ...
                _logger.warning(
                    "DomainOrg pk=%s has no user, group, or org_id; skipping role assignment.",
                    domain_org.pk,
                )
                continue
        # Always create org group role if org_id is present (separate from user/group roles)
        org_group = None
        if domain_org.org_id:
            org_group, _ = Group.objects.get_or_create(name=f"rh-org-{domain_org.org_id}")
        ...
        for domain in domains:
            ...
            if org_group is not None and org_group != group:
                _assign_pair(GroupRole, {"group": org_group}, domain_owner_role, admin_role, domain, domain_ct)
    ...
    for domain_name, policy in getattr(settings, "DOMAIN_ACCESS_POLICIES", {}).items():
        ...
        domain = Domain.objects.filter(name=domain_name).only("pk").first()
        if domain is None:
            _logger.warning(
