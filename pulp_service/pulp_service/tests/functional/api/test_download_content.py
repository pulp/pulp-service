...
from pulp_service.tests.functional.constants import SERVICE_FIXTURE_URL
from pulp_service.tests.functional.utils import (
    gen_service_client,
    get_service_content_paths,
    gen_service_remote,
)
...
from pulpcore.client.pulp_service import (
    ...
    PublicationsServiceApi,
    RepositoriesServiceApi,
    RepositorySyncURL,
    RemotesServiceApi,
    ServiceServicePublication,
)


# Implement tests.functional.utils.get_service_content_unit_paths(),
...
class DownloadContentTestCase(unittest.TestCase):
    ...
    def test_all(self):
        ...
        * `Pulp Smash #872 <https://github.com/pulp/pulp-smash/issues/872>`_
        ...
        client = gen_service_client()
        repo_api = RepositoriesServiceApi(client)
        remote_api = RemotesServiceApi(client)
        publications = PublicationsServiceApi(client)
        distributions = DistributionsServiceApi(client)
        ...
        publish_data = ServiceServicePublication(repository=repo.pulp_href)
        ...
        unit_paths = [choice(paths) for paths in get_service_content_paths(repo.to_dict()).values()]
        fixtures_hashes = [
            hashlib.sha256(utils.http_get(urljoin(SERVICE_FIXTURE_URL, unit_path[0]))).hexdigest()
