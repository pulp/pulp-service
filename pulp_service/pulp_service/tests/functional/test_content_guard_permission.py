...
def configure_pypi_distribution(
    ...
    def _configure(domain_name, features=None):
        with anonymous_user:
            # Under RBAC non-admins create domains via the self-service endpoint, not
...
def test_non_pypi_endpoints_unaffected_by_content_guard(configure_guarded_pypi_distribution):
    ...
    Under PulpServiceAccessPolicy non-PyPI endpoints use standard object-level RBAC, so
