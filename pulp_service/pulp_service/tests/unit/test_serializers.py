...
from pulp_service.app.serializers import ServiceContentSerializer
from pulp_service.app.models import ServiceContent

from pulpcore.plugin.models import Artifact


# Fill data with sufficient information to create ServiceContent
# Provide sufficient parameters to create the ServiceContent object
# Depending on the base class of the serializer, provide either "_artifact" or "_artifacts"
@unittest.skip("FIXME: plugin writer action required")
class TestServiceContentSerializer(TestCase):
    """Test ServiceContentSerializer."""

    def setUp(self):
        """Set up the ServiceContentSerializer tests."""
    ...
    def test_valid_data(self):
        """Test that the ServiceContentSerializer accepts valid data."""
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = ServiceContentSerializer(data=data)
        self.assertTrue(serializer.is_valid())

    def test_duplicate_data(self):
        """Test that the ServiceContentSerializer does not accept data."""
        ServiceContent.objects.create(artifact=self.artifact)
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = ServiceContentSerializer(data=data)
