...
# Expose pulp-content service
oc expose svc pulp-content --path=/api/pulp-content --port 8000
PULP_CONTENT_ROUTE=http://$(oc get route pulp-content -ojsonpath='{.spec.host}')
...
debug_and_fail() {
  ...
  echo "CURL OUTPUT"
  curl http://${PULP_CONTENT_ROUTE}/api/pulp-content/default/
  echo "ROUTES"
  oc get route
  exit 1
}
...
cmd_prefix bash -c "HOME=/tmp/home pip3 install pulpcore-client pulp_file-client pulp_rpm-client pulp_ostree-client"

cmd_prefix mkdir -p /tmp/home/.config/pulp_smash
sed "s#password#${PASSWORD}#g" pulp-smash.json > pulp-smash.customized.json
sed -i "s/pulp-content/env-${NAMESPACE}.apps.crc-eph.r9lp.p1.openshiftapps.com/g" pulp-smash.customized.json
...
cmd_prefix bash -c "CUSTOM_CONTENT_ORIGIN=$PULP_CONTENT_ROUTE HOME=/tmp/home PYTHONPATH=/tmp/home/.local/lib/python3.9/site-packages/ XDG_CONFIG_HOME=/tmp/home/.config API_PROTOCOL=http API_HOST=pulp-api API_PORT=8000 ADMIN_USERNAME=admin ADMIN_PASSWORD=$PASSWORD /tmp/home/.local/bin/pytest -o cache_dir=/tmp/home/.cache/pytest_cache -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_rpm.tests.functional -m parallel -n 8 -k 'test_download_content' --junitxml=/tmp/home/junit-pulp-parallel.xml" || debug_and_fail
