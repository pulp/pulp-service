...
COPY images/assets/patches/0033-Adds-redis-WORKER_TYPE.patch /tmp/
RUN patch -p1 -d /usr/local/lib/pulp/lib/python${PYTHON_VERSION}/site-packages < /tmp/0033-Adds-redis-WORKER_TYPE.patch
...
COPY images/assets/patches/0047-Re-enable-attestation-verification-with-vendored-key.patch /tmp/
RUN patch -p1 -d /usr/local/lib/pulp/lib/python${PYTHON_VERSION}/site-packages < /tmp/0047-Re-enable-attestation-verification-with-vendored-key.patch
ENV PULP_ATTESTATION_VERIFICATION_KEY=/etc/pki/sigstore/SIGSTORE-redhat-release3
