...
func OpenJiraIssue(result, question string) string {
	...
	return fmt.Sprintf(
		"Based on the following analysis, you need to create a Jira issue or add a comment to an existing one — but first check the preconditions and existing issues.\n\n"+
			...
			"    a) Use the jira_get_comments tool to retrieve all existing comments on that issue.\n"+
			"    b) Check whether any existing comment already describes the same observations from this analysis.\n"+
			"    c) If NO existing comment covers these observations, use the jira_add_comment tool to add a comment summarizing the new findings from the analysis,\n"+
			"       noting how they relate to (but differ from) the original issue description.\n"+
			"       If the comment contains sensitive information (see Step 3), set its visibility to \"Red Hat Employee\" so it is not publicly visible.\n"+
			"    d) If a comment with substantially the same content already exists, do NOT add a duplicate. Report the existing issue key and the existing comment.\n"+
			...
			"    If the issue contains sensitive information (see Step 3), set the security level to make it visible only to \"Red Hat Employee\".\n\n"+
			...
			"  If ANY sensitive information is present, the issue or comment MUST be created as private (visible only to \"Red Hat Employee\").\n\n"+
			"--- Analysis ---\n%s\n--- End of Analysis ---\n\n"+
