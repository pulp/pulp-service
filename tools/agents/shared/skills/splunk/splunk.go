...
const SPLUNK_TRAFFIC_QUERY = `index=federated:rh_akamai sourcetype=datastream:access (reqHost=*packages*redhat.com OR reqHost=*console*redhat.com) (reqPath=api/pulp* OR reqPath=api/pypi* OR reqPath=v2* OR reqPath=extensions/v2* OR reqPath=trusted-libraries* OR reqPath=lightwell*)
| eval base_path=mvjoin(mvindex(split(reqPath, "/"), 0, 2), "/")
| eval path_type=case(match(reqPath, "^api/pulp-content"), "Content", match(reqPath, "^api/pulp-container"), "Container", match(reqPath, "^api/pulp"), "API", match(reqPath, "^api/pypi"), "PyPI", match(reqPath, "^v2") OR match(reqPath, "^extensions/v2"), "Container", match(reqPath, "^trusted-libraries"), "PyPI", match(reqPath, "^lightwell"), "Lightwell", true(), "Other")
| stats count by reqHost, base_path, path_type
| sort -count`
...
const maxJobWait = 20 * time.Minute
	...
	for {
		...
		case <-ticker.C:
			if time.Now().After(deadline) {
				return fmt.Errorf("splunk job %s did not complete within %v", sid, maxJobWait)
			}
